...
```json
{
  ...
  "definitions": {
    ...
    "Category": {
      "type": "object",
      "description": "USDMの「カテゴリ」。要求仕様書全体の構成を示す。上位要求のグループ。",
    ...
    "SubRequirement": {
      "type": "object",
      "description": "USDMの「下位要求」。上位要求を具体化・分割した要求。",
      "required": ["id", "requirement", "reason"],
      "properties": {
        "id": {
          "type": "string",
...
| 8 | カテゴリ | `category.id` + `category.name` |
...
| 要素 | ID形式 | パターン | 例 |
|------|--------|----------|----|
| Category | `CXX` | `^[A-Za-z][A-Za-z0-9_-]*$` | C01, C02 |
| Requirement | `RXX` | 同上 | R01, R02, R11 |
| SubRequirement | `RXX-YY` | 同上 | R01-01, R01-02 |
| Specification | `SXX-YY-ZZ` | 任意文字列 | S01-01-01, spc11-01-01 |

> **注**: IDの命名規約はプロジェクトごとに決定してよい。スキーマはパターン `^[A-Za-z][A-Za-z0-9_-]*$` を要求する（Specificationを除く）。
...
```json
{
  "version": "1.0.0",
  "title": "サンプル",
  "categories": [{ "id": "C01", "name": "機能", "requirements": [] }],
  "requirementRelations": [
    {
      "sourceId": "R01",
      "targetId": "R02",
...
| ID制約 | `^[A-Za-z][A-Za-z0-9_-]*$`（英字始まり） |
| グループ名 | `^＜.+＞$`（全角山括弧） |
| 仕様ラベル | `verified: [boolean, boolean, boolean]`（3要素固定） |
| 階層パターン | `specificationGroups` / `requirementGroups` の選択（同時保持も可） |
